...
export const useDomainDiagramsStore = create((set, get) => ({
  ...
  fetch: async (domainId) => {
    ...
    try {
      const response = await api.getDomainDiagrams(domainId);
      ...
      const message =
        err?.response?.data?.message || "Failed to load domain diagrams";
  ...
  analyze: async (domain, includeDocumentation = true) => {
    if (!domain?.id) return { success: false, error: "Invalid domain" };

    useLogsStore.getState().clearLogs(domain.id, SECTION_TYPES.DIAGRAMS);
    ...
    try {
      await api.analyzeDomainDiagrams(
        domain.id,
        domain.files || [],
      ...
      const message =
        err?.response?.data?.message || "Failed to analyze diagrams";
