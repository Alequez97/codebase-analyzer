...
export async function queueDesignGeneratePageTask({
  ...
  const agentConfigResult = getAgentConfig(TASK_TYPES.DESIGN_GENERATE_PAGE, model);
  ...
  const task = {
    ...
    params: {
      ...
      htmlOutputPath: getDesignHtmlOutputPath(normalizedDesignId, normalizedPageId),
      cssOutputPath: getDesignCssOutputPath(normalizedDesignId, normalizedPageId),
