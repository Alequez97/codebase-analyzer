...
import { getDesignReverseEngineerPageSystemInstructionPath } from "../../../constants/design-system-instructions.js";
...
export async function queueDesignReverseEngineerPageTask({
  designId,
  pageId,
  pageName,
  route = "",
  ...
  const taskId = generateTaskId(TASK_TYPES.DESIGN_REVERSE_ENGINEER_PAGE);
  ...
  const task = {
    id: taskId,
    type: TASK_TYPES.DESIGN_REVERSE_ENGINEER_PAGE,
    ...
    systemInstructionFile:
      getDesignReverseEngineerPageSystemInstructionPath(technology),
    progressFile: getProgressFileRelativePath(taskId),
  };

  await initChatHistory(taskId, {
    taskType: TASK_TYPES.DESIGN_REVERSE_ENGINEER_PAGE,
  });
