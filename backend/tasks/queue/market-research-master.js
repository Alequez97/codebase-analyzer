...
/**
 * Queue a market research master task.
 * @param {Object} params
 ...
 */
export async function queueMarketResearchMasterTask({ sessionId, idea } = {}) {
  if (!sessionId || !idea) {
    return { success: false, error: "sessionId and idea are required" };
  }

  const agentConfigResult = getAgentConfig(TASK_TYPES.MARKET_RESEARCH_MASTER);
  if (!agentConfigResult.success) {
    return agentConfigResult;
  }

  const agentConfig = agentConfigResult.agentConfig;
  const taskId = generateTaskId(TASK_TYPES.MARKET_RESEARCH_MASTER);

  const task = {
    id: taskId,
    type: TASK_TYPES.MARKET_RESEARCH_MASTER,
    ...
    agentConfig,
    systemInstructionFile: SYSTEM_INSTRUCTION_PATHS.MARKET_RESEARCH_MASTER,
  ...
  logger.info("Queued market research master task", {
    component: "MarketResearchQueue",
    taskId,
    sessionId,
  });
