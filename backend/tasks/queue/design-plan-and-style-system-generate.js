...
export async function queueDesignPlanAndStyleSystemGenerateTask({
  ...
  const agentConfigResult = getAgentConfig(TASK_TYPES.DESIGN_PLAN_AND_STYLE_SYSTEM_GENERATE, model);
  if (!agentConfigResult.success) {
    return agentConfigResult;
  }

  const normalizedDesignId = slugifyDesignId(designId || prompt);
  const taskId = generateTaskId(TASK_TYPES.DESIGN_PLAN_AND_STYLE_SYSTEM_GENERATE);
  const task = {
    id: taskId,
  ...
  await initChatHistory(taskId, { taskType: TASK_TYPES.DESIGN_PLAN_AND_STYLE_SYSTEM_GENERATE });
